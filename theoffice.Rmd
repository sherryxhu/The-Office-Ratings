...
included_characters <- c("Michael", "Dwight", "Jim", "Pam", "Ryan", "Andy", "Robert", "Angela", "Stanley", "Kevin", "Creed", "Meredith", "Kelly", "Erin", "Toby", "Oscar", "Darryl", "Phyllis", "Gabe", "Holly", "Jan", "Roy", "Todd Packer", "David Wallace", "Karen", "Bob Vance", "Nellie")
...
```{r}
# initial model 
data_ratings <- data[c(1:57,60:64)]
#data_rating <- data%>%seplyr::select_se(data, )
m1 <- lm(rating ~ episode + season + lines_Michael + lines_Jim + lines_Pam + lines_Dwight + s_Michael + s_Jim + s_Pam + s_Dwight , data = data_ratings)
# m1 <- lmer(rating ~ . + (1|season), data = data)
summary(m1)
```
...
# Forward Selection
```{r model1}
# initial forward selection
bigger.model <- formula(lm(rating ~ season + episode + lines_Michael + lines_Jim + lines_Dwight + lines_Pam + lines_Nellie + s_Michael + s_Pam + s_Dwight + s_Jim + lines_Stanley + s_Stanley + lines_Nellie + s_Nellie + lines_Toby + s_Toby + lines_Gabe + s_Gabe + lines_Ryan + s_Ryan + lines_Karen + s_Karen + lines_Robert + s_Robert + lines_Andy + s_Andy, data_ratings))

v1.model <- step(m1, direction = "forward", scope = list(lower = m1, upper = bigger.model))

interactions <- formula(lm(rating ~ season + episode + lines_Michael + lines_Jim + lines_Dwight + lines_Nellie + s_Michael + s_Pam + s_Dwight + lines_Stanley + s_Stanley + lines_Nellie + lines_Toby + s_Toby  + lines_Karen + s_Gabe + s_Andy + s_Ryan + season*lines_Michael + season*s_Michael + season*lines_Pam + season*lines_Jim + season*s_Jim + season*lines_Dwight + season*s_Dwight , data_ratings))

v2.model <- step(v1.model, direction = "forward", scope = list(lower = v1.model, upper = interactions))

writers <- formula(lm(rating ~ season + episode + lines_Michael + lines_Jim + lines_Dwight + lines_Pam + lines_Nellie + s_Michael + s_Pam + s_Dwight + s_Jim + lines_Stanley + s_Stanley + lines_Nellie + lines_Toby + s_Ryan + lines_Karen + s_Toby + s_Gabe + s_Andy + season*s_Dwight + Mindy_Kaling + B.J._Novak + Paul_Lieberstein + Michael_Schur + episode, data_ratings))
